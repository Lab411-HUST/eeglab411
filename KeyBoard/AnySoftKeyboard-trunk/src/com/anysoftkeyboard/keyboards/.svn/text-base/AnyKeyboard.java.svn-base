...
public abstract class AnyKeyboard extends Keyboard
{
                ...
                switch (primaryCode)
                {
                    case KeyCodes.QUICK_TEXT:
                        QuickTextKey quickKey = QuickTextKeyFactory
                                .getCurrentQuickTextKey(getASKContext());
                        ...
                        Resources quickTextKeyResources = quickKey.getPackageContext()
                                .getResources();
                    ...
                    default:
                        ...
                        if (isAlphabetKey(key) && (key.icon == null))
                        {
                             ...
                             * anyKey.shiftedCodes.length > 1) { //HO, the label
                             * is not generated, it was specified, and this is a
                             * multi chars key anyKey.shiftedKeyLabel =
                             * anyKey.label.toString().toUpperCase(); } else {
                             * final char code = (char)anyKey.shiftedCodes[0];
                             * //check the ASCII table, everything below 32, is
                             * not printable if (code > 31 &&
    ...
    protected void addGenericRows(AnyKeyboardContextProvider askContext, Context context, int mode,
        ...
        if (!mBottomRowWasCreated)
        {
            ...
            KeyboardMetadata bottomMd = addKeyboardRow(bottomRowPlugin.getPackageContext(),
    ...
    private void skipToEndOfRow(XmlResourceParser parser) throws XmlPullParserException,
            IOException
    {
        int event;
        while ((event = parser.next()) != XmlResourceParser.END_DOCUMENT) {
        ...
        if ((key.codes != null) && (key.codes.length > 0))
        {
            ...
            switch (primaryCode)
            {
                ...
                case KeyCodes.ENTER:// enter
                    key = mEnterKey = new EnterKey(mASKContext, res, parent, keyboardDimens, x, y,
                            parser);
                    break;
                case KeyCodes.SHIFT:
    ...
    public void setImeOptions(Resources res, EditorInfo editor) {
        if (AnyApplication.DEBUG)
        {
            if (editor == null)
                ...
                Log.d(TAG, "AnyKeyboard.setImeOptions. package: " + editor.packageName + ", id:"
                        + editor.fieldId);
        }
