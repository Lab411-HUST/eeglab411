...
public class PDFReaderAct extends Activity implements OnItemClickListener,
		OnClickListener, PDFReaderListener, PDFThumbListener {
	...
	final static String RIGHT = "com.lab411.gazeright";
	final static String LEFT = "com.lab411.gazeleft";
	final static String BLINK = "com.lab411.eyeblink";
	final static String DOWN = "com.lab411.gazedown";
	int CurrentPage = 0;
	int focusID = 1;
	boolean zoomState=false;
	Animation BlinkAnimation;
	
	Button BtnNext, BtnPrevious, BtnZoom;
	BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(BLINK)) {
				switch(focusID){
				case 1:if(CurrentPage>0)m_reader.PDFGotoPage(--CurrentPage) ;break;
				case 2:if(zoomState)m_reader.PDFzoom(540, 400, -2);else m_reader.PDFzoom(540, 400, 2); zoomState=!zoomState; ;break;
				case 3:if(CurrentPage<PageCount-1)m_reader.PDFGotoPage(++CurrentPage) ;break;
				}
			}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		m_layout = (RelativeLayout) LayoutInflater.from(this).inflate(
		...
		m_vFiles.PDFSetRootPath("/mnt/sdcard/");
		m_vFiles.setOnItemClickListener(this);
		setContentView(m_vFiles);

		LinearLayout bar_cmd = (LinearLayout) m_layout
				.findViewById(R.id.bar_cmd);
		LinearLayout bar_act = (LinearLayout) m_layout
				.findViewById(R.id.bar_act);
		LinearLayout bar_find = (LinearLayout) m_layout
				.findViewById(R.id.bar_find);
		...
		m_doc = new Document();
		m_doc.Open("/mnt/sdcard/abc.pdf", null);

		m_reader.PDFOpen(m_doc, false, this);
		m_thumb.thumbOpen(m_doc, this);
		setContentView(m_layout);

		PageCount = m_doc.GetPageCount();

		m_reader.PDFGotoPage(CurrentPage);
		
		BlinkAnimation = AnimationUtils.loadAnimation(this, R.anim.fade_in);
		focusID = 1;
		...
		BtnNext.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(CurrentPage<PageCount-1)m_reader.PDFGotoPage(++CurrentPage);
			}
		});
		...
		BtnPrevious.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				if(CurrentPage>0)m_reader.PDFGotoPage(--CurrentPage);
			}
		});
		...
		BtnZoom.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				if(zoomState)m_reader.PDFzoom(540, 400, -2);else m_reader.PDFzoom(540, 400, 2); zoomState=!zoomState;
				focusChange(true);

			}
		});
		BtnPrevious.startAnimation(BlinkAnimation);
	...
	public void focusChange(boolean right) {
		if (right) {
			switch (focusID) {
			case 1: focusID++;BtnPrevious.clearAnimation();BtnZoom.startAnimation(BlinkAnimation);break;
			case 2: focusID++;BtnZoom.clearAnimation();BtnNext.startAnimation(BlinkAnimation);break;
			case 3: focusID=1;BtnNext.clearAnimation();BtnPrevious.startAnimation(BlinkAnimation);break;			
			}
		} else {
			switch (focusID) {
			case 1: focusID=3;BtnPrevious.clearAnimation();BtnNext.startAnimation(BlinkAnimation);break;
			case 2: focusID--;BtnZoom.clearAnimation();BtnPrevious.startAnimation(BlinkAnimation);break;
			case 3: focusID--;BtnNext.clearAnimation();BtnZoom.startAnimation(BlinkAnimation);break;
			}
		}
	...
	protected void onResume() {
		registerReceiver(receiver, new IntentFilter(RIGHT));
		registerReceiver(receiver, new IntentFilter(LEFT));
		registerReceiver(receiver, new IntentFilter(BLINK));
		registerReceiver(receiver, new IntentFilter(DOWN));
		super.onResume();
	}
	...
	private void InputPassword(PDFGridItem item) {
		LinearLayout layout = (LinearLayout) LayoutInflater.from(this).inflate(
				R.layout.dlg_password, null);
		final EditText tpassword = (EditText) layout
	...
	private void InitView() {
		m_doc.SetCache(Global.tmp_path + "/temp.dat");// set temporary cache for
														// editing.
		// test font delegate
	...
	public void OnSelectEnd(String text) {
		LinearLayout layout = (LinearLayout) LayoutInflater.from(this).inflate(
				R.layout.dlg_text, null);
		final RadioGroup rad_group = (RadioGroup) layout
		...
		builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				if (rad_group.getCheckedRadioButtonId() == R.id.rad_copy)
					Toast.makeText(PDFReaderAct.this,
							"todo copy text:" + sel_text, Toast.LENGTH_SHORT)
							.show();
				else if (m_reader.PDFCanSave()) {
					boolean ret = false;
					if (rad_group.getCheckedRadioButtonId() == R.id.rad_copy) {
						Toast.makeText(PDFReaderAct.this,
								"todo copy text:" + sel_text,
								Toast.LENGTH_SHORT).show();
						android.content.ClipboardManager clipboard = (android.content.ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
						android.content.ClipData clip = android.content.ClipData
								.newPlainText("Radaee", sel_text);
					...
					if (!ret)
						Toast.makeText(PDFReaderAct.this,
								"add annotation failed!", Toast.LENGTH_SHORT)
								.show();
				} else
					Toast.makeText(PDFReaderAct.this,
							"can't write or encrypted!", Toast.LENGTH_SHORT)
							.show();
				onSelect();
				dialog.dismiss();
			}
