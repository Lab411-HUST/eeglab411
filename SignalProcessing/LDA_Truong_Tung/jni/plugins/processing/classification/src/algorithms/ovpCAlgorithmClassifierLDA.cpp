...
boolean CAlgorithmClassifierLDA::train(const IFeatureVectorSet& rFeatureVectorSet)
{
	...
	//vector <double>l_oMeanFeatureVector1(l_ui32NumberOfFeatures);
	//vector <double> l_oMeanFeatureVector2(l_ui32NumberOfFeatures);
    cv::Mat l_oMeanFeatureVector1(l_ui32NumberOfFeatures,1,CV_64F);
    cv::Mat l_oMeanFeatureVector2(l_ui32NumberOfFeatures,1,CV_64F);
	...
	for(i=0; i<rFeatureVectorSet.getFeatureVectorCount(); i++)
	{
		...
		if(l_f64Label==m_f64Class1)
		{
			...
			l_oMeanFeatureVector1=l_oMeanFeatureVector1+ std::vector<l_rFeatureVector.getBuffer(),l_rFeatureVector.getSize()>;
		}

		if(l_f64Label==m_f64Class2)
		{
		//	l_oMeanFeatureVector2+=itpp::vec(l_rFeatureVector.getBuffer(), l_rFeatureVector.getSize());
			l_oMeanFeatureVector2+=std::vector(l_rFeatureVector.getBuffer(),l_rFeatureVector.getSize());
		}
	}
	...
	vector <double> l_oDiff;
	Mat l_oSigma;
	l_oSigma=Mat::zeros(l_ui32NumberOfFeatures, l_ui32NumberOfFeatures,CV_64F);
	Mat l_oSigm;
	l_oSigm=Mat::zeros(l_ui32NumberOfFeatures,l_ui32NumberOfFeatures,CV_64F);
	for(i=0; i<rFeatureVectorSet.getFeatureVectorCount(); i++)
	{
		const IFeatureVector& l_rFeatureVector=rFeatureVectorSet[i];
		...
		//	l_oDiff=itpp::vec(l_rFeatureVector.getBuffer(), l_rFeatureVector.getSize()) - l_oMeanFeatureVector1;
			l_oDiff=std::vector(l_rFeatureVector.getBuffer(),l_rFeatureVector.getSize())-l_oMeanFeatureVector1;
			//l_oSigma+=itpp::outer_product(l_oDiff, l_oDiff) / (l_ui32NumberOfFeatureVectors-2.);

			l_oSigma=l_oDiff*l_oDiff;
		}
		...
		//	l_oSigma+=itpp::outer_product(l_oDiff, l_oDiff) / (l_ui32NumberOfFeatureVectors-2.);
			l_oDiff=std::vector(l_rFeatureVector.getBuffer(), l_rFeatureVector.getSize())-l_oMeanFeatureVector2;
			l_oSigma=l_oDiff*l_oDiff/(l_ui32NumberOfFeatureVectors-2.);
		}

	}

	cv::Mat l_oSigmaInverse=l_oSigma.inv();
    cv::MatExpr x;
    Eigen
...
boolean CAlgorithmClassifierLDA::classify(const IFeatureVector& rFeatureVector, float64& rf64Class, IVector& rClassificationValues)
{
	...
	vector <double>l_oFeatures(rFeatureVector.getBuffer(), rFeatureVector.getSize());
    printf("%d",rFeatureVector.getBuffer()[1]);
	//l_oFeatures.ins(0, 1);
   // l_oFeatures.insert(0,1);
